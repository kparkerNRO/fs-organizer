...
def create_test_entries(session: Session):
    ...
    test_entries = [
        # Tower examples
        GroupCategoryEntry(
            folder_id=1, iteration_id=0, processed_name="Castle Tower", 
            pre_processed_name="Castle Tower", path="/test1", confidence=0.8
        ),
        GroupCategoryEntry(
            folder_id=2, iteration_id=0, processed_name="Wizard Tower", 
            pre_processed_name="Wizard Tower", path="/test2", confidence=0.8
        ),
        GroupCategoryEntry(
            folder_id=3, iteration_id=0, processed_name="Tower Interior", 
            pre_processed_name="Tower Interior", path="/test3", confidence=0.8
        ),
        
        # Location type examples
        GroupCategoryEntry(
            folder_id=1, iteration_id=0, processed_name="Village Tavern", 
            pre_processed_name="Village Tavern", path="/test1", confidence=0.8
        ),
        GroupCategoryEntry(
            folder_id=2, iteration_id=0, processed_name="City Tavern", 
            pre_processed_name="City Tavern", path="/test2", confidence=0.8
        ),
        
        # Collaboration examples
        GroupCategoryEntry(
            folder_id=1, iteration_id=0, processed_name="Collaboration with Fantasy Atlas", 
            pre_processed_name="Collaboration with Fantasy Atlas", path="/test1", confidence=0.8
        ),
        GroupCategoryEntry(
            folder_id=2, iteration_id=0, processed_name="Collaboration with Cze Peku", 
            pre_processed_name="Collaboration with Cze Peku", path="/test2", confidence=0.8
        ),
        
        # Single word tags (should not be decomposed)
        GroupCategoryEntry(
            folder_id=1, iteration_id=0, processed_name="Interior", 
            pre_processed_name="Interior", path="/test1", confidence=0.8
        ),
        GroupCategoryEntry(
            folder_id=2, iteration_id=0, processed_name="Maps", 
            pre_processed_name="Maps", path="/test2", confidence=0.8
        ),
    ]
...
def test_component_statistics_extraction():
    ...
    with sessionmaker() as session:
        ...
        print(f"Top words: {dict(sorted(stats.word_frequencies.items(), key=lambda x: x[1], reverse=True)[:5])}")
...
def test_decomposition_candidate_generation():
    ...
    with sessionmaker() as session:
        ...
        for candidate in candidates[:5]:  # Show first 5
            print(f"  '{candidate.original_tag}' -> {candidate.components} "
...
if __name__ == "__main__":
    # Run tests manually
    print("Testing component statistics extraction...")
    test_component_statistics_extraction()
    
    print("\nTesting decomposition candidate generation...")
    test_decomposition_candidate_generation()
