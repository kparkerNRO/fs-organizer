...
def test_correct_spelling(input_entries, expected_entries):
    ...
    for name, entry in input_entries.items():
        assert entry.grouped_name == expected_entries[name].grouped_name
        assert entry.confidence == expected_entries[name].confidence
        assert entry.categories[0] == expected_entries[name].categories[0]
