...
HIGH_FREQUENCY_THRESHOLD = 3  # Words appearing this many times are considered significant
...
class TagDecomposer:
    ...
    def extract_component_statistics(self, entries: List[GroupCategoryEntry]) -> ComponentStatistics:
    ...
    def compute_tag_vectors_and_embeddings(self) -> Tuple[Dict[str, np.ndarray], np.ndarray]:
    ...
    def calculate_semantic_score(self, original_tag: str, components: List[str]) -> float:
        ...
        for i, comp1 in enumerate(components):
            ...
            for j, comp2 in enumerate(components):
                ...
                comp2_contexts = self.component_stats.word_contexts.get(comp2_lower, set())
    ...
    def extract_ngram_components(self, max_n: int = 4) -> Tuple[Dict[str, List[Tuple[List[str], float]]], Dict[str, int]]:
        ...
        for tag in self.vocabulary:
            if len(tag.split()) > 1:  # Only consider multi-word tags
                potential_components = self._find_component_combinations(tag, frequent_ngrams)
                if potential_components:
                    decomposition_candidates[tag] = potential_components
        
        return decomposition_candidates, frequent_ngrams
    
    def _find_component_combinations(self, tag: str, components: Dict[str, int]) -> List[Tuple[List[str], float]]:
    ...
    def _score_decomposition(self, original_tag: str, components: List[str], component_frequencies: Dict[str, int]) -> float:
    ...
    def calculate_structural_score(self, original_tag: str, components: List[str]) -> float:
    ...
    def calculate_embedding_score(self, original_tag: str, components: List[str]) -> float:
    ...
    def calculate_cooccurrence_score(self, original_tag: str, components: List[str]) -> float:
    ...
    def calculate_pattern_score(self, original_tag: str, components: List[str]) -> float:
    ...
    def analyze_hierarchical_patterns(self) -> Dict[str, Dict]:
        ...
        for tag in sorted_tags:
            ...
            for existing_tag in sorted_tags:
                ...
                if (len(existing_words) < len(words) and 
                    words[:len(existing_words)] == existing_words):
        ...
        for base_tag, extensions in prefix_groups.items():
            if len(extensions) >= MIN_HIERARCHICAL_GROUP_SIZE:
                ...
                hierarchical_candidates[f"hierarchical:{base_tag}"] = {
                    ...
                    "confidence": min(1.0, len(extensions) / 5.0)  # Higher confidence with more extensions
                }
                    ...
                    for component in split_components:
                        if component in self.tag_to_idx:
                            ...
                            try:
                                ...
                                logger.debug(f"Failed to generate embedding for component '{component}': {e}")
                                continue
        ...
        for tag in self.vocabulary:
            if len(tag.split()) >= 2:
                ...
                for component in possible_components:
                    if component in self.component_stats.cooccurrence_matrix and component != tag:
                        comp_coocs = self.component_stats.cooccurrence_matrix[component]
            ...
            for extended_tag in info["extended_tags"]:
                if extended_tag in self.vocabulary:
                    ...
                    if len(extension_words) > 1:
                        ...
                        frequent_extension_words = sum(1 for freq in extension_word_frequencies if freq >= 2)
                        if frequent_extension_words >= len(extension_words) * 0.7:  # 70% of words are frequent
                        ...
                        if len(extension_words) == 2:
                            ...
                            word1_freq = self.component_stats.word_frequencies.get(extension_words[0], 0)
                            word2_freq = self.component_stats.word_frequencies.get(extension_words[1], 0)
                    ...
                    if extended_tag in combined_candidates:
                        ...
                        combined_candidates[extended_tag]["evidence"].append("hierarchical")
                        combined_candidates[extended_tag]["confidence"] += 0.6  # High boost for hierarchical
                        # Update decomposition if hierarchical is better
                        combined_candidates[extended_tag]["decompositions"] = [(components, info["confidence"])]
                    else:
    ...
    def generate_decomposition_candidates(self, entries: List[GroupCategoryEntry]) -> List[DecompositionCandidate]:
        ...
        for normalized_tag, info in candidate_dict.items():
            ...
            for entry_id in entry_ids:
                ...
                semantic_score = self.calculate_semantic_score(normalized_tag, components)
                structural_score = self.calculate_structural_score(normalized_tag, components)
                embedding_score = self.calculate_embedding_score(normalized_tag, components)
                cooccurrence_score = self.calculate_cooccurrence_score(normalized_tag, components)
                ...
                if overall_confidence >= self.min_confidence:
                    candidate = DecompositionCandidate(
                        ...
                        pattern_score=pattern_score + hierarchical_score,  # Include hierarchical in pattern score
                    ...
                    logger.debug(f"Multi-modal candidate: '{entry.processed_name.strip()}' -> {components}")
                    logger.debug(f"  Confidence: {overall_confidence:.3f}, Evidence: {info['evidence']}")
                    logger.debug(f"  Scores - freq: {frequency_score:.3f}, embed: {embedding_score:.3f}, "
    ...
    def create_decomposed_entries(self, candidates: List[DecompositionCandidate], 
                                 session: Session, iteration_id: int) -> List[GroupCategoryEntry]:
        ...
        for entry_id, entry_candidates in entry_to_candidates.items():
            ...
            for component in best_candidate.components:
                new_entry = GroupCategoryEntry(
                    ...
                    derived_names=(original_entry.derived_names or []) + [best_candidate.original_tag],
            ...
            logger.debug(f"Multi-modal decomposition: '{best_candidate.original_tag}' -> {best_candidate.components}")
            logger.debug(f"  Confidence: {best_candidate.overall_confidence:.3f}, Evidence: {best_candidate.evidence_types}")
            logger.debug(f"  Scores - freq: {best_candidate.frequency_score:.3f}, embed: {best_candidate.embedding_score:.3f}, "
...
def decompose_compound_tags(session: Session) -> None:
    ...
    decomposer.tag_vectors, decomposer.embeddings = decomposer.compute_tag_vectors_and_embeddings()
    
    logger.info(f"Found {len(decomposer.component_stats.word_frequencies)} unique component words "
    ...
    if not candidates:
        ...
        new_entries = decomposer.create_decomposed_entries(candidates, session, iteration_id)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python tag_decomposition.py <db_path>")
        sys.exit(1)
