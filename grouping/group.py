...
def heuristic_categorize(db_path: Path, update_table: bool = False) -> None:
    ...
    try:
        ...
        non_variant_folders = session.query(Folder).filter(
            Folder.classification != ClassificationType.VARIANT
        ).all()
        ...
        for folder in non_variant_folders:
            if len(folder.categories) == 1:
                children = session.query(Folder).filter(
                    Folder.parent_path.startswith(folder.folder_path)
                ).all()
                if len(children) == 0 or all(child.classification == ClassificationType.VARIANT for child in children):
...
def group_categories(db_path: Path, threshold: int = 80) -> dict[int, list[str]]:
    ...
    try:
        ...
        for name in distinct_names:
            ...
            for group in groups:
                if any(calculate_similarity_difflib(name, member, threshold) for member in group):
                    group.append(name)
                    break
            else:
                groups.append([name])
            ...
            if new_grouping:
                for name, group_entry in new_grouping.items():
                    session.query(ProcessedName).filter(
                        ...
                        ProcessedName.grouped_name: ",".join(group_entry.categories),
...
def calculate_and_process_groups(db_path: Path, threshold: int = 80) -> dict[int, list[str]]:
...
def process_pre_calculated_groups(db_path: Path) -> None:
    ...
    try:
        # Get existing groups
        groups = (
            session.query(ProcessedName.group_id, ProcessedName.folder_name)
            .all()
        )
