...
export const FolderBrowser: React.FC<FolderBrowserProps> = ({
  ...
  // Helper function to get FolderItem background color
  const getFolderItemBgColor = (
    isDraggedOver: boolean,
    isSelected: boolean,
    isInHighlightedPath: boolean
  ): string => {
    if (isDraggedOver) return "#e2f0fd";
    if (isSelected) return "#dbeafe";
    if (isInHighlightedPath) return "#f0f9ff";
    return "transparent";
  };

  // Helper function to get FolderItem border color
  const getFolderItemBorderColor = (
    isDraggedOver: boolean,
    isSelected: boolean,
    isInHighlightedPath: boolean
  ): string => {
    if (isDraggedOver) return "#3b82f6";
    if (isSelected) return "#60a5fa";
    if (isInHighlightedPath) return "#e0f2fe";
    return "transparent";
  };
  ...
  const renderNode = (
    ...
    return (
      <div key={nodePath}>
        <div
          className={`flex items-center w-full box-border rounded-sm transition-all duration-150 my-[0.25px] text-[0.8125rem] leading-[1.3] border relative ${
            isDraggedOver ? "border-dashed" : "border-solid"
          } ${isFile ? "text-gray-600" : "text-black"}`}
          style={{
            paddingTop: "0.1875rem",
            paddingBottom: "0.1875rem",
            paddingRight: "0.3rem",
            paddingLeft: `${level * 0.625 + 0.3}rem`,
            cursor: isDragging ? "grabbing" : "pointer",
            opacity: isDragging ? 0.5 : 1,
            backgroundColor: getFolderItemBgColor(
              isDraggedOver,
              isHighlighted,
              isInSelectedPath
            ),
            borderColor: getFolderItemBorderColor(
              isDraggedOver,
              isHighlighted,
              isInSelectedPath
            ),
          ...
          onMouseEnter={(e) => {
            ...
            if (isHighlighted) {
              target.style.backgroundColor = "#c7d2fe";
            } else {
              target.style.backgroundColor = "#e7eefa";
            }
          }}
          onMouseLeave={(e) => {
            const target = e.currentTarget;
            target.style.backgroundColor = getFolderItemBgColor(
              isDraggedOver,
              isHighlighted,
              isInSelectedPath
            );
          ...
          onMouseDown={(e) => {
            const target = e.currentTarget;
            target.style.backgroundColor = "#dce6f8";
          }}
          onMouseUp={(e) => {
            const target = e.currentTarget;
            if (isHighlighted) {
        ...
        >
          {!isFile && hasChildren ? (
            <span
              className="inline-flex items-center justify-center mr-0.75 w-3 h-3"
          ...
          <span
            className="font-medium whitespace-nowrap overflow-hidden text-ellipsis flex-1 px-1.5 py-0.5 rounded-sm mr-1"
            style={{
              display: "flex",
              alignItems: "center",
              backgroundColor:
                !isFile && showConfidence && (node as FolderV2).confidence !== undefined
                  ? getConfidenceBackgroundColor((node as FolderV2).confidence)
                  : "transparent",
            }}
          >
            {renamingItem &&
            ...
            renamingItem.item === node ? (
              <form onSubmit={handleRenameSubmit} style={{ flex: 1 }}>
                <input
                  type="text"
                  className="w-full bg-white border-2 border-blue-500 rounded-sm px-1 py-0.5 text-[0.8125rem] font-medium outline-none focus:border-blue-700"
              ...
              >
                ...
                {!isFile && hasLowConfidenceChildFolders(node as FolderV2) && (
                  <div className="w-1.5 h-1.5 bg-red-600 rounded-full ml-1 flex-shrink-0" />
                )}
              </span>
            )}
            ...
            {showConfidence &&
              ...
              (node as FolderV2).confidence !== undefined && (
                <span className="text-[0.7rem] font-normal text-[rgb(51,55,61)] ml-auto pl-2">
          ...
          {isFile && (node as File).fileType ? (
            <span className="ml-2 text-gray-500 text-xs bg-gray-100 px-1.5 py-0.5 rounded">
              {(node as File).fileType}
            </span>
          ) : null}
          ...
          {!isFile && (node as FolderV2).path && nodePath !== "root" ? (
            <span className="ml-2 text-gray-500 text-xs whitespace-nowrap overflow-hidden text-ellipsis max-w-[180px]">
              {(node as FolderV2).path}
            </span>
          ) : null}
        </div>
  ...
  return (
    <FolderBrowserErrorBoundary>
      <div className="w-full bg-white rounded shadow-sm p-1 box-border overflow-hidden flex flex-col flex-1 min-h-0">
        {treeState.tree ? (
          <>
            <div
              ref={scrollContainerRef}
              className="mt-0.5 flex-1 overflow-y-auto min-h-0 w-full"
              style={{
                scrollbarWidth: "thin",
              }}
            >
              <style>
                {`
                  .mt-0\\.5::-webkit-scrollbar {
                    width: 4px;
                  }
                  .mt-0\\.5::-webkit-scrollbar-track {
                    background: #f1f1f1;
                    border-radius: 2px;
                  }
                  .mt-0\\.5::-webkit-scrollbar-thumb {
                    background: #c1c1c1;
                    border-radius: 2px;
                  }
                  .mt-0\\.5::-webkit-scrollbar-thumb:hover {
                    background: #a1a1a1;
                  }
                `}
              </style>
