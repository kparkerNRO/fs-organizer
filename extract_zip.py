...
def extract_zip_without_hidden_files(zipref:zipfile.ZipFile, final_path:Path):
    ...
    for member in zipref.infolist():
        ...
        try:
            ...
            logger.warning(f"Bad Zip file for file {filename}")
            continue
...
def extract_zip(
    ...
    logger.info(f"target dir: {out_dir_name}")
    ...
    with zipfile.ZipFile(zip_file, "r") as zipref:
        ...
        if preserve_modules and "module.json" in zip_children:
            logger.info(f"File {filename} is a foundry module, moving to {module_dir}")
            final_path = try_move_file(
                zip_file,
                Path(module_dir),
...
@click.option("--backup_dir", "-b", default="")
